...
//  Aisha
//
//  Created by Mohamed Rashwan on 22/04/2018.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [tabBarController setViewControllers:
      @[prayersNavigationController,
        //activitiesNavigationController,
        settingsNavigationController]];//, logNavigationController]];
     ...
     */
    NSString *correctVersion = @"1.0.3 (1)";
    ...
    if(existingVersion == nil || ![existingVersion isEqualToString:correctVersion])
    {
        ...
        UIAlertView *welcome = [[UIAlertView alloc] initWithTitle:@"Assalamu Alikum <1.0.3 (1)>" message:@"Salam, if you choose to play athan from settings then please make sure to change the app notification settings on the phone from Temporary to Persistent. This will allow the athan audio to play completely to the end." delegate:self cancelButtonTitle:@"JAK" otherButtonTitles:nil , nil];
    ...
    if([self.settingsViewController.fajrSwitch isOn])
    {
        firstReminderDate = [self createReminderForPrayerName:@"Fajr"
                                                   ...
                                                   withAmOrPm:@"am"
                                                    ...
                                                    extraNote:nil];//[NSString stringWithFormat:@"Sunrise at %@", prayerTimes.sunriseAzan]];
    }
...
- (NSDate*)createReminderForPrayerName:(NSString *)prayerName atTime:(NSString *)prayerTime withAmOrPm:(NSString *)ampm jamaaTime:(NSString *)jamaaTime extraNote:(NSString *)extraNote {
    ...
    NSDate *date = [prayerTimesGate getPrayerTimeAt:prayerTime ampm:ampm];
    ...
    */
    ...
    for (int i=0; i<1; i++)
    {
        ...
        else if([date dateByAddingTimeInterval:60 * 60 * 24 * i] == nil)
        ...
        [self.logViewController addLog:[NSString stringWithFormat:@"registering date %@",[inputFormatter stringFromDate:[date dateByAddingTimeInterval:60 * 60 * 24 * i] ]]];
        
        fireDate = [date dateByAddingTimeInterval:60 * 60 * 24 * i];
